...
class DatabaseManager:
    ...
    def get_match_between_users(self, user_one_id: int, user_two_id: int) -> Optional[schemas.Match]:
        """Проверить существование мэтча между двумя пользователями"""
        with self.create_session() as db:
            user_min = min(user_one_id, user_two_id)
            user_max = max(user_one_id, user_two_id)

            return db.query(schemas.Match) \
                .filter_by(
                user_one_id=user_min,
                user_two_id=user_max,
                is_active=True
            ).first()
    ...
    def suggest_benches_for_match(
        ...
        with self.create_session() as db:
            ...
            if not (user_a and user_b):
                logger.error("Users not found for match")
                return []

            if not all([user_a.latitude, user_a.longitude, user_b.latitude, user_b.longitude]):
                logger.warning("Missing coordinates for users")
                return []
